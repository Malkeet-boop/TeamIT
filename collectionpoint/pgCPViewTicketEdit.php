...
$qry2="select collection_point_name,collection_point_logo,mobile_number from tw_collection_point_master where id='".$sender_id."' ";
$retVal2=$sign->FunctionJson($qry2);
$decodedJSON2=json_decode($retVal2);
$collection_point_name=$decodedJSON2->response[0]->collection_point_name;
$collection_point_logo=$decodedJSON2->response[1]->collection_point_logo;
$mobile_number=$decodedJSON2->response[2]->mobile_number;
...
<title>Trace Waste |Collection Point Tickets</title>
<!-- plugins:css -->
				...
				<div class="col-md-12 grid-margin stretch-card">
					<div class="card">
						<div class="card-body">
							...
							<small><span class="ti-calendar"></span> <?php echo date("d-m-Y H:i:s",strtotime($ticket_date));?> | <span class="ti-user"></span> <?php echo $collection_point_name; ?></small>
							<hr>
							<p><?php echo $description; ?> </p>
							<?php if($view_files!=""){ ?>
								<hr>
								<a href="<?php echo $settingValueCollectionPointImagePathVerification.$mobile_number."/".$view_files ?>" target="__blank"><i class="icon-paper menu-icon"></i> View File</a>
							<?php }?>
						</div>
					</div>
				</div>
			...
			<div class="row">
					<div class="col-md-12 grid-margin stretch-card">
						<div class="card">
							...
							<div class="row">
								<div class="col-md-12">
										<?php if ($reply_by=="COLLECTIONPOINT") { ?>
										<div class="row">
											<div class="col-lg-8 offset-lg-3 col-md-8 offset-md-3 col-sm-10 col-xs-10 col-10 chat-right">
												<small><span class="ti-calendar"></span><?php echo date("d-m-Y H:i:s",strtotime($reply_date));  ?> | <span class="ti-user"></span> <?php echo $collection_point_name; ?></small>
												<hr>
												<p><?php echo $reply; ?></p>
												
												<?php if (!empty($reply_attachment)) { ?>
												<a href="<?php echo $settingValueCollectionPointImagePathVerification.$mobile_number."/".$reply_attachment; ?>" target="__blank"><i class="ti-download"> Attachment</i></a>
												
												<?php } ?>
											</div>
											<div class="col-lg-1 col-md-1 col-sm-2 col-xs-2 col-2">
											<?php if (!empty($collection_point_logo)) { ?>
												<img src="<?php echo $settingValueCollectionPointImagePathVerification.$mobile_number."/".$collection_point_logo; ?>" class="img-sm rounded-circle mb-3" />
											<?php }else{?>
												<img src="<?php echo $settingValueCollectionPointImagePathOther.$settingValueCollectionPointImage; ?>" class="img-sm rounded-circle mb-3" />
										...
										<div class="row">
											...
											<div class="col-lg-8 col-md-8 col-sm-10 col-xs-10 col-10 chat-left">
												...
												<?php if (!empty($reply_attachment)) { ?>
					<a href="<?php echo $settingValueAdminImagePathSupport.'CollectionPoint'."/".$reply_attachment; ?>" target="__blank"><i class="ti-download"> Attachment</i></a>
												<?php } ?>
											</div>
										</div>
										<?php } ?>
...
var hdnIDimg="";
var mobile_number="<?php echo $mobile_number; ?>";
$('input').blur(function()
{
...
var path = "<?php echo $settingValueCollectionPointImagePathVerification; ?>"+mobile_number+"/"+name;
...
function addrecord(){
		...
		disableButton('#btnAddrecord','<i class="ti-timer"></i> Processing...');
			...
			$.ajax({
				type:"POST",
				url:"apiAddCPReplyTicketMaster.php",
				...
				success:function(response){
					...
					if($.trim(response)=="Success"){
						showAlertRedirect("Success","Reply Posted Successfully","success","pgCPViewTicketEdit.php");
