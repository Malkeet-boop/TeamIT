...
$SScompany_id=$_SESSION["cp_id"];
$cp_id=$_SESSION["cp_id"];

$qry="Select ts.id,ts.ticket_date,ts.subject,vsm.verification_status from tw_ticket_system ts INNER JOIN tw_verification_status_master vsm ON ts.status=vsm.id where ts.status='".$settingValueCompletedStatus."' and ts.sender_id='".$cp_id."' and ts.panel='CollectionPoint'";

$qry1="select count(*) as cnt from tw_ticket_system where status='".$settingValueCompletedStatus."' and sender_id='".$cp_id."' and panel='CollectionPoint'";
